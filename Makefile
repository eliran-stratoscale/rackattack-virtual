...
COVERED_FILES=rackattack/common/hoststatemachine.py,rackattack/common/hosts.py,rackattack/common/dnsmasq.py
unittest:
	UPSETO_JOIN_PYTHON_NAMESPACES=Yes PYTHONPATH=. python -m coverage run -m unittest $(UNITTESTS)
	python -m coverage report --show-missing --rcfile=coverage.config --fail-under=79 --include=$(COVERED_FILES)
